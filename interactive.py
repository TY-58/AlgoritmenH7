...
grid_version: int = get_int("")

configuration_version: int = get_int("Firstly, which configuration do you want to run: random (type 1) or greedy (type 2)?")

cable_route_version: int = get_int("And which cable route version do you want to use: random without shared cables (type 1), greedy without shared cables(type 2) or greedy with shared cables (type 3)?")
